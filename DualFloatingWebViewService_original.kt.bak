...
import com.example.aifloatingball.service.WebViewToolbarService
...
class DualFloatingWebViewService : FloatingServiceBase(), WindowStateCallback {
    ...
    private fun executeScriptInActiveWebView(script: String) {
        if (!::webViewManager.isInitialized) {
            Log.w(TAG, "WebViewManager not initialized, cannot execute script")
            return
        }
        
        handler.post {
            try {
                // 获取所有WebView并在可见的WebView中执行脚本
                ...
                // 如果没有可见的WebView，在第一个WebView中执行
                if (!executed && webViews.isNotEmpty()) {
                    webViews[0]?.evaluateJavascript(script) { result ->
                        Log.d(TAG, "脚本执行结果(fallback): $result")
                    }
                }
                
                Log.d(TAG, "已执行JavaScript脚本")
            } catch (e: Exception) {
                Log.e(TAG, "执行JavaScript脚本失败", e)
            }
        }
    }
        ...
        intent?.let {
            ...
            if (searchParams != null && searchParams.query.isNotEmpty()) {
                ...
                if (showToolbar) {
                    // 延迟显示工具栏，等待WebView加载
                    handler.postDelayed({
                        val toolbarIntent = Intent(this, WebViewToolbarService::class.java).apply {
                            action = WebViewToolbarService.ACTION_SHOW_TOOLBAR
                            putExtra(WebViewToolbarService.EXTRA_SEARCH_TEXT, searchParams.query)
                            putExtra(WebViewToolbarService.EXTRA_AI_ENGINE, searchParams.engineKey ?: "Unknown")
                        }
