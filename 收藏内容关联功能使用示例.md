...
collectionManager.addRelation(
    ...
    note = "collection_1依赖于collection_3",
...
relatedItems.forEach { (item, relation) ->
    println("关联项: ${item.title}, 类型: ${relation.relationType.displayName}")
}
...
```kotlin
// 查找从collection_1到collection_5的路径
...
val relatedByContent = collectionManager.findRelatedByContent(
    itemId = "collection_1",
    similarityThreshold = 0.3f  // 相似度阈值30%
)

relatedByContent.forEach { (item, similarity) ->
    println("${item.title}: 相似度 ${(similarity * 100).toInt()}%")
}
```
...
val flowchart = generator.generateFlowchart(
    ...
    relations = relations,
    rootId = "collection_1"  // 可选：指定根节点
)

// 生成HTML页面
val html = generator.generateHtmlPage(flowchart, "收藏项关联图")
```
...
class KnowledgeGraphExample {
    ...
    fun createKnowledgeGraph(context: Context) {
        ...
        // 3. 建立关联关系
        // item1 包含 item2 和 item3
        manager.addRelation(
            sourceId = item1.id,
            targetId = item2.id,
            relationType = RelationType.CONTAINS,
            note = "Android开发需要了解Kotlin"
        )
        ...
        // 6. 在WebView中展示
        val html = generator.generateHtmlPage(flowchart, "知识图谱")
        // ... 加载到WebView
    }
}
```
...
manager.addRelation(
    ...
    note = "引用了article_2的内容"
)
```

## 📊 性能优化建议

1. **批量操作**：如果需要添加大量关联，考虑批量处理
2. **缓存**：对于频繁查询的关联关系，可以使用缓存
3. **索引**：如果数据量大，考虑使用数据库索引
4. **异步处理**：关联查询和图表生成可以在后台线程进行
...
fun detectAllCycles(manager: UnifiedCollectionManager) {
    ...
    collections.forEach { item ->
        item.relations.forEach { relation ->
            ...
            if (hasCycle) {
                println("发现循环: ${item.title} -> ${relation.targetId}")
            }
        }
    }
}
