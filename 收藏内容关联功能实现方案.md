...
- 单个收藏项可以关联多个其他收藏项
- 支持双向关联
- 支持关联类型（如：相关、引用、依赖、包含等）
- 可视化展示为思维导图或Mermaid图表
- 支持关联关系的增删改查

---

## 🎯 方案一：数据模型内嵌关联字段（推荐用于快速实现）
...
/**
 * 扩展UnifiedCollectionItem，添加关联字段
 */
...
class CollectionRelationManager private constructor(private val context: Context) {
    ...
    private fun saveRelations(relations: List<CollectionRelationEntity>): Boolean {
        return try {
            ...
            Log.d(TAG, "保存关联关系成功: ${relations.size} 条")
            true
        } catch (e: Exception) {
            Log.e(TAG, "保存关联关系失败", e)
            false
        }
    }
...
data class GraphNode(
    ...
    val neighbors: MutableSet<String> = mutableSetOf()  // 邻居节点ID
)
...
**推荐库：**
- `com.github.PhilJay:MPAndroidChart`（图表库，可用于网络图）
- `com.github.anastr:speedviewlib`（自定义View）
- 自定义Canvas绘制
...
| 方案 | 复杂度 | 性能 | 灵活性 | 适用场景 |
|------|--------|------|--------|----------|
| **方案一：内嵌字段** | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | 快速实现，中小规模数据 |
| **方案二：独立关系表** | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 复杂关联，需要元数据 |
| **方案三：图数据库** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 大规模数据，复杂查询 |
| **方案四：标签关联** | ⭐ | ⭐⭐⭐ | ⭐⭐ | 简单场景，自动发现 |
...
### 阶段一：基础实现（方案一）
1. 扩展`UnifiedCollectionItem`添加`relations`字段
2. 扩展`UnifiedCollectionManager`添加关联管理方法
3. 实现基础的关联增删改查

### 阶段二：可视化实现
1. 实现Mermaid图表生成器
2. 在WebView中展示Mermaid图表
3. 添加思维导图视图（可选）

### 阶段三：优化升级（方案二）
1. 如果数据量大或需要复杂查询，迁移到独立关系表
2. 添加关联关系的元数据支持
3. 实现关联路径查找、社区发现等功能

### 阶段四：高级功能（方案三）
1. 如果需要图算法支持，实现图数据库结构
2. 添加中心度分析、最短路径等功能
3. 实现智能推荐（基于关联关系）
...
1. **数据一致性**：删除收藏项时必须清理所有关联关系
2. **循环检测**：添加关联时检测是否形成循环
3. **性能优化**：大量关联时考虑使用索引或缓存
4. **用户体验**：提供清晰的关联可视化界面
5. **数据迁移**：如果切换方案，需要提供数据迁移工具
