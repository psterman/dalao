...
class SimpleModeService : Service() {
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
                    ...
                    if (!query.isNullOrEmpty()) {
                        settingsManager.setDisplayMode("floating_ball")
                            val serviceIntent =
                                    Intent(context, DualFloatingWebViewService::class.java).apply {
    ...
    private fun setupMinimizedView() {
        minimizedView.findViewById<LinearLayout>(R.id.minimized_layout).setOnTouchListener {
                view,
                event ->
            when (event.action) {
    ...
    private fun snapToEdge() {
        minimizedParams.x =
                if (minimizedParams.x < resources.displayMetrics.widthPixels / 2) -20
                else resources.displayMetrics.widthPixels - 20
            windowManager.updateViewLayout(minimizedView, minimizedParams)
    }
    ...
    private fun setupViews() {
        ...
        simpleModeView.findViewById<ImageButton>(R.id.simple_mode_minimize_button)
                .setOnClickListener { minimizeToEdge() }
        simpleModeView.findViewById<ImageButton>(R.id.simple_mode_close_button).setOnClickListener {
            stopSelf()
        }
        ...
        simpleModeView.findViewById<LinearLayout>(R.id.tab_home).setOnClickListener {
            resetToIntentGrid()
        }
        ...
        simpleModeView.findViewById<LinearLayout>(R.id.tab_voice).setOnClickListener {
            hideWindow()
            startActivity(
                    Intent(this, VoiceRecognitionActivity::class.java)
    ...
    private fun showIntentGrid() {
        ...
        promptTemplates.values.forEach { template ->
            val gridItem =
                    createGridItem(template.intentName, template.icon) {
                ...
                EditText(this).apply {
                    ...
                    background =
                            ContextCompat.getDrawable(
                                    context,
                                    R.drawable.search_bar_background_simple
                            )
                }
        gridLayout.addView(
                editText,
    ...
    private fun collectAndProceed() {
        ...
        when (field.type) {
            FieldType.TEXT_INPUT -> {
                data =
                        gridLayout
                                .findViewById<EditText>(R.id.dynamic_edit_text)
                                .text
                                .toString()
                                .trim()
            }
            ...
            FieldType.MULTIPLE_CHOICE -> {
                val container =
                        gridLayout.findViewById<LinearLayout>(R.id.dynamic_checkbox_container)
                val selected =
    ...
    private fun generateAndExecuteFinalPrompt() {
        ...
        if (profile.persona.isNullOrBlank() ||
                        profile.tone.isNullOrBlank() ||
                        profile.outputFormat.isNullOrBlank()
        ) {
            wasCancelled = false
            showPersonaSetupDialog(template, data)
            return
        }

        executePromptWithProfile(template, data, profile)
    }

    private fun showPersonaSetupDialog(template: PromptTemplate, data: MutableMap<String, Any>) {
        // *** THE FIX IS HERE ***
        // Use the correct ServiceDialogTheme instead of the base app theme
        val context = ContextThemeWrapper(this, R.style.ServiceDialogTheme)
        val customView = LayoutInflater.from(context).inflate(R.layout.dialog_persona_prompt, null)

        val dialog = AlertDialog.Builder(context).setView(customView).setCancelable(true).create()

        customView.findViewById<Button>(R.id.dialog_positive_button).setOnClickListener {
            val intent =
                    Intent(this, MasterPromptSettingsActivity::class.java).apply {
                        flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_SINGLE_TOP
                        putExtra("auto_focus_persona", true)
                    }
            startActivity(intent)
            Toast.makeText(this, "请先完善您的个人信息", Toast.LENGTH_LONG).show()
            minimizeToEdge()
            dialog.dismiss()
        }

        customView.findViewById<Button>(R.id.dialog_negative_button).setOnClickListener {
            val defaultProfile = com.example.aifloatingball.model.PromptProfile.DEFAULT
            executePromptWithProfile(template, data, defaultProfile)
            Toast.makeText(this, "好的，先用默认设置为您搜索", Toast.LENGTH_SHORT).show()
            dialog.dismiss()
        }

        customView.findViewById<Button>(R.id.dialog_neutral_button).setOnClickListener {
            wasCancelled = true
            Toast.makeText(this, "已取消，您可以重新选择", Toast.LENGTH_SHORT).show()
            resetToIntentGrid()
            dialog.dismiss()
        }
    ...
    private fun executePromptWithProfile(
            template: PromptTemplate,
            data: MutableMap<String, Any>,
            profile: com.example.aifloatingball.model.PromptProfile
    ) {
        var finalPrompt =
                "请基于以下设定为我提供信息：我的角色是\"${profile.persona}\"，我希望内容的语气是\"${profile.tone}\"，并且输出格式为\"${profile.outputFormat}\"。\n---\n"
    ...
    private fun directSearch(
            query: String,
            source: String = "直接搜索",
            engines: Array<String> = arrayOf("deepseek", "google", "bing")
    ) {
        Toast.makeText(this, "正在为您搜索...", Toast.LENGTH_SHORT).show()
        val intent =
                Intent(this, DualFloatingWebViewService::class.java).apply {
    ...
    private fun createGridItem(title: String, icon: String, onClick: () -> Unit): View {
        val gridItemView =
                LayoutInflater.from(this)
                        .inflate(R.layout.simple_mode_grid_item, gridLayout, false) as
                        LinearLayout
        ...
        return mapOf(
                                ...
                                fields =
                                        listOf(
                                                ...
                                                PromptField(
                                                        ...
                                                        "希望了解的深度是？",
                                                        FieldType.SINGLE_CHOICE,
                                                        options = listOf("入门介绍", "深入细节", "专家视角")
                                                )
                                        ),
                                finalPromptFormat =
                                        "请以“{depth}”的水平，帮我介绍一下“{topic}”。如果可能，请侧重于“{aspects}”方面进行说明。",
                ...
                "solve" to
                        PromptTemplate(
                                ...
                                finalPromptFormat =
                                        "我遇到了一个问题：“{problem}”。具体情况是：“{context}”。我的目标是：“{goal}”。请提供详细的解决方案步骤。",
                ...
                "choose" to
                        PromptTemplate(
                                ...
                                finalPromptFormat =
                                        "我正在“{options}”之间做选择。我主要的评判标准是“{criteria}”，其中“{priority}”是优先考虑的。请帮我分析各项优劣并给出建议。",
                ...
                "plan" to
                        PromptTemplate(
                                ...
                                finalPromptFormat =
                                        "请帮我为“{activity}”制定一个为期“{duration}”的详细计划。计划需要考虑“{participants}”。",
                ...
                "discuss" to
                        PromptTemplate(
                                ...
                                finalPromptFormat =
                                        "我想和你深入讨论一下“{topic}”。我的初步看法是：“{viewpoint}”。请分享你的观点，并提出一些有深度的问题。",
                                recommendedEngines = listOf("deepseek")
                        ),
                "analyze" to
                        PromptTemplate(
