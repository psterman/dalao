...
public class AiChatWidgetProvider extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.ai_chat_widget_layout);

        // 创建点击意图
        Intent intent = new Intent(context, SimpleModeActivity.class);
        intent.putExtra("widget_type", "ai_chat");
        intent.putExtra("source", "桌面小组件");
        intent.putExtra("search_mode", "ai_chat");
        ...
        // 设置整个小组件的点击事件
        views.setOnClickPendingIntent(R.id.ai_chat_widget_layout, pendingIntent);
